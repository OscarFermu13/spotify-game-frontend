  ...
  return (
    <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-green-500/40 via-emerald-500/20 to-slate-900 p-[1px]">
      <div className="relative rounded-2xl bg-slate-900/80 p-6 overflow-hidden">
        <div className="absolute -right-6 -top-6 w-36 h-36 rounded-full bg-green-400/8 blur-2xl pointer-events-none" />
          ...
          <div className="flex-shrink-0">
            {daily?.alreadyCompleted ? (
              <button
                ...
                className="px-4 py-2.5 bg-green-500 hover:bg-green-400 text-black text-sm font-bold rounded-xl transition-all hover:scale-105 active:scale-95"
              >
                🏆 Ranking
              </button>
            ) : (
              <button
                onClick={onPlay}
                disabled={!daily}
                className="px-5 py-2.5 bg-green-500 hover:bg-green-400 text-black text-sm font-bold rounded-xl transition-all hover:scale-105 active:scale-95 disabled:opacity-40 disabled:cursor-not-allowed"
              >
                ▶ Jugar
              </button>
            )}
...
function SectionLabel({ children }) {
  return (
    <div className="flex items-center gap-3 mb-4">
      <span className="text-xs font-bold tracking-widest text-slate-500 uppercase">{children}</span>
...
export default function App() {
  ...
  if (authChecking) {
    return (
      <div className="min-h-screen bg-slate-950 flex items-center justify-center">
        <style>{`@import url('https://fonts.googleapis.com/css2?family=Syne:wght@700;800&family=DM+Sans:wght@300;400;500&display=swap'); @keyframes eq { from { transform: scaleY(0.3); } to { transform: scaleY(1); } }`}</style>
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white" style={{ fontFamily: "'DM Sans', sans-serif" }}>
      ...
      {/* Noise texture — identical to SessionPlay / Leaderboards */}
      ...
      <div className="relative max-w-3xl mx-auto px-6 py-8 pb-16">
        ...
        {/* ── Header — identical structure to SessionPlay / Leaderboards ── */}
          ...
          {isAuthenticated ? (
            <div className="flex flex-col items-end gap-1">
              ...
              <div className="flex items-center gap-2">
                <button onClick={() => handleLogin(true)} className="text-xs text-slate-600 hover:text-slate-400 transition">
                  Cambiar cuenta
                </button>
                <span className="text-slate-800">·</span>
                <button onClick={handleLogout} className="text-xs text-slate-600 hover:text-slate-400 transition">
                  Salir
                </button>
              </div>
            </div>
            ...
            <div className="space-y-3">
                      ...
                      {selectedPlaylist ? (
                        <span className="flex items-center gap-2">
                          {selectedPlaylist.images?.[0] && (
                            <img src={selectedPlaylist.images[0].url} className="w-5 h-5 rounded" alt="" />
                          )}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <select value={count} onChange={(e) => setCount(Number(e.target.value))} disabled={loading} className={selectCls}>
                ...
                <div>
                  ...
                  <div className="relative">
                    <input
                      type="number" min="1" value={penalty}
        ...
        <section className="mb-10">
          ...
          {!isAuthenticated && (
            <p className="text-xs text-slate-600 mt-2 ml-1">
              Necesitas conectar Spotify para jugar →{' '}
              <button onClick={() => handleLogin(false)} className="text-green-500 hover:text-green-400 transition underline underline-offset-2">
                Conectar
              </button>
            </p>
          )}
